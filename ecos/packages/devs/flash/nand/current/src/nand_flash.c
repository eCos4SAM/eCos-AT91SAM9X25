...
// 
//      ./devs/flash/nand/current/src/nand_flash.c 
//========================================================================== 
...
//               
// 
// This driver modify for NAND FLASH with large page sizes
// 
...
{ 
    cyg_nand_dev* dev = &nand_device; // ADD AXONIM Devices
    ...
    if(column >= 0) 
    { 
        if((dev->page_size == 256)||(dev->page_size == 512)) 
        {
            CYGHWR_FLASH_NAND_PLF_ADDR(column); 
        } else if(dev->page_size == 2048)
        {
            CYGHWR_FLASH_NAND_PLF_ADDR(column & 0xFF);
            CYGHWR_FLASH_NAND_PLF_ADDR((column >> 8) & 0xFF);
        }
    } 
    ...
    {
        CYGHWR_FLASH_NAND_PLF_CMD(NAND_CMD_READ2);
    }
    if(cmd == NAND_CMD_RDRND0)
    {
        CYGHWR_FLASH_NAND_PLF_CMD(NAND_CMD_RDRND1);
    }     
...
{ 
    ...
    res = FLASH_ERR_OK;
    if(res == FLASH_ERR_OK) DEBUG_PRINT_HDW("Succeed ERASE\n"); 
...
{ 
    ...
    // ADD.AXONIM large device
    else if(dev->page_size == 2048) 
    { 
        nand_check_ecc(data, spare[0x28] | (spare[0x29] << 8) | (spare[0x2A] << 16)); 
        nand_check_ecc(data + 256, spare[0x2B] | (spare[0x2C] << 8) | (spare[0x2D] << 16)); 
        nand_check_ecc(data + 512, spare[0x2E] | (spare[0x2F] << 8) | (spare[0x30] << 16)); 
        nand_check_ecc(data + 768, spare[0x31] | (spare[0x32] << 8) | (spare[0x33] << 16)); 
        nand_check_ecc(data + 1024, spare[0x34] | (spare[0x35] << 8) | (spare[0x36] << 16)); 
        nand_check_ecc(data + 1280, spare[0x37] | (spare[0x38] << 8) | (spare[0x39] << 16)); 
        nand_check_ecc(data + 1536, spare[0x3A] | (spare[0x3B] << 8) | (spare[0x3C] << 16)); 
        nand_check_ecc(data + 1792, spare[0x3D] | (spare[0x3E] << 8) | (spare[0x3F] << 16)); 
    }
    // ADD.AXONIM large device
    else 
    { 
...
{ 
    ...
    { 
        ecc = nand_calc_ecc(data); 
        spare[0x28] = ecc & 0xFF; 
        spare[0x29] = (ecc >> 8) & 0xFF; 
        spare[0x2A] = (ecc >> 16) & 0xFF; 
         
        ecc = nand_calc_ecc(data + 256); 
        spare[0x2B] = ecc & 0xFF; 
        spare[0x2C] = (ecc >> 8) & 0xFF; 
        spare[0x2D] = (ecc >> 16) & 0xFF; 

        ecc = nand_calc_ecc(data + 512); 
        spare[0x2E] = ecc & 0xFF; 
        spare[0x2F] = (ecc >> 8) & 0xFF; 
        spare[0x30] = (ecc >> 16) & 0xFF; 

        ecc = nand_calc_ecc(data + 768); 
        spare[0x31] = ecc & 0xFF; 
        spare[0x32] = (ecc >> 8) & 0xFF; 
        spare[0x33] = (ecc >> 16) & 0xFF; 

        ecc = nand_calc_ecc(data + 1024); 
        spare[0x34] = ecc & 0xFF; 
        spare[0x35] = (ecc >> 8) & 0xFF; 
        spare[0x36] = (ecc >> 16) & 0xFF; 

        ecc = nand_calc_ecc(data + 1280); 
        spare[0x37] = ecc & 0xFF; 
        spare[0x38] = (ecc >> 8) & 0xFF; 
        spare[0x39] = (ecc >> 16) & 0xFF; 

        ecc = nand_calc_ecc(data + 1536); 
        spare[0x3A] = ecc & 0xFF; 
        spare[0x3B] = (ecc >> 8) & 0xFF; 
        spare[0x3C] = (ecc >> 16) & 0xFF; 

        ecc = nand_calc_ecc(data + 1792); 
        spare[0x3D] = ecc & 0xFF; 
        spare[0x3E] = (ecc >> 8) & 0xFF; 
        spare[0x3F] = (ecc >> 16) & 0xFF; 
    } 
    // ADD.AXONIM large device
    else 
    { 
    ...
    if(res == FLASH_ERR_OK) DEBUG_PRINT_HDW("Succeed WRITE\n"); 
    else DEBUG_PRINT_HDW("Failed (status = 0x%X)\n", status); 
...
{ 
    ...
    if (dev->page_size == 2048) 
    {
        ...
        nand_command(NAND_CMD_RDRND0, 2048, -1); 
        CYGHWR_FLASH_NAND_PLF_WAIT();
        nand_read_buf(dev->buf_spare, dev->spare_size); 
     
        CYGHWR_FLASH_NAND_PLF_CE(0); 

        if(dev->buf_spare[0x00] != 0xFF) res = 1; 
        else 
        { 
...
DEBUG_PRINT_CMD("nand_bbm_getpage\n"); 
...
{ 
    ...
    //diag_printf("Initialize NAND flash\n");
