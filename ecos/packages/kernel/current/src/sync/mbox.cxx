...
// Mboxes may optionally be placed on a housekeeping list so that all mboxes may be located easily. This is useful mainly in conjunction with source-level debugging.

#ifdef CYGVAR_KERNEL_SYNCOBJ_LIST

Cyg_Mbox *Cyg_Mbox::mbox_list = 0;

inline void
Cyg_Mbox::add_to_list( void )
{
    Cyg_Scheduler::lock();
    ...
    if( mbox_list == 0 )
        ...
        Cyg_Mbox *prev = mbox_list;
        do {
            if ( this == prev )
                break; // found it already!
            prev = prev->list_next;
        } while ( prev != mbox_list );
        if ( this != prev ) {
            // insert it in the list:
            list_next = mbox_list->list_next;
            mbox_list->list_next = this;
        }
    }
    mbox_list = this;
...
inline void
Cyg_Mbox::remove_from_list( void )
{
    Cyg_Scheduler::lock();
...
{
#ifdef CYGVAR_KERNEL_SYNCOBJ_LIST
    // Add mutexes to housekeeping list
    add_to_list();
#endif    
}
