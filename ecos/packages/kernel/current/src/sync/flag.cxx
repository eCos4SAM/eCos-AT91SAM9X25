...
// Flags may optionally be placed on a housekeeping list so that all flags may be located easily. This is useful mainly in conjunction with source-level debugging.

#ifdef CYGVAR_KERNEL_SYNCOBJ_LIST

Cyg_Flag *Cyg_Flag::flag_list = 0;

inline void
Cyg_Flag::add_to_list( void )
{
    Cyg_Scheduler::lock();
    ...
    if( flag_list == 0 )
        ...
        Cyg_Flag *prev = flag_list;
        do {
            if ( this == prev )
                break; // found it already!
            prev = prev->list_next;
        } while ( prev != flag_list );
        if ( this != prev ) {
            // insert it in the list:
            list_next = flag_list->list_next;
            flag_list->list_next = this;
        }
    }
    flag_list = this;
...
inline void
Cyg_Flag::remove_from_list( void )
{
    Cyg_Scheduler::lock();
...
#ifdef CYGVAR_KERNEL_SYNCOBJ_LIST
    // Remove flag from housekeeping list
    remove_from_list();
#endif    
