...
// Mutexes may optionally be placed on a housekeeping list so that all mutexes may be located easily. This is useful mainly in conjunction with source-level debugging.

#ifdef CYGVAR_KERNEL_SYNCOBJ_LIST

Cyg_Mutex *Cyg_Mutex::mutex_list = 0;

inline void
Cyg_Mutex::add_to_list( void )
{
    Cyg_Scheduler::lock();
    ...
    if( mutex_list == 0 )
        ...
        Cyg_Mutex *prev = mutex_list;
        do {
            if ( this == prev )
                break; // found it already!
            prev = prev->list_next;
        } while ( prev != mutex_list );
        if ( this != prev ) {
            // insert it in the list:
            list_next = mutex_list->list_next;
            mutex_list->list_next = this;
        }
    }
    mutex_list = this;
...
inline void
Cyg_Mutex::remove_from_list( void )
{
    Cyg_Scheduler::lock();
...
#ifdef CYGVAR_KERNEL_SYNCOBJ_LIST
    // Add mutex to housekeeping list
    add_to_list();
#endif    
...
#ifdef CYGVAR_KERNEL_SYNCOBJ_LIST
    // Remove mutex from housekeeping list
    remove_from_list();
#endif    
